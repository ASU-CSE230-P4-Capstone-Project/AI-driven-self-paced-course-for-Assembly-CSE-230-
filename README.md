...
```bash
# register
curl -i -X POST https://capstonep3-x86emulator.vercel.app/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{"username":"admin@asu.edu","password":"admin123","role":"admin","adminKey":"<ADMIN_SETUP_KEY>"}'

# login
curl -i -X POST https://capstonep3-x86emulator.vercel.app/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin@asu.edu","password":"admin123","role":"admin"}'
```