...
pub fn decode(bytes: &[u8]) -> Result<Instruction, DecodeError> {
    ...
    match opcode {
        ...
        Opcode::CMP => {
            // CMP r/m32, r32 (0x39): rm = dest, reg = src
            if bytes.len() < 2 {
                return Err(DecodeError::InsufficientBytes);
            }
            ...
            // 0x39 CMP r/m32, r32: rm field = destination, reg field = source
            let dest_reg = reg_from_index(modrm & 0x7);
            let src_reg  = reg_from_index((modrm >> 3) & 0x7);
